  ...
  var loading = $("#loader");
    ...
    if (a=='N'){
      ...
      $.ajax({
        type:"GET",
        url:"proses.php",
        data:"aksi=proses_update_dicetak&id="+b+"&dicetak="+cek,
...
if ($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  <div class="grid">
                ...
                <div class="grid-title">
                 <div class="pull-left">
                    ...
                    <span style="width: 300px;">Nama yang dicetak</span> 
                    <div class="clearfix"></div>
                 </div>
                 <div class="pull-right"> 
               ...
               <form id='form_data' method='get' action='<?php echo "media.php?".$_SESSION['query_string']?>'> 
                   ...
                   <input type="hidden" name="module" value="nama_dicetak" />
                   Tampilkan: <select name='dicetak' id='dicetak'>
                    <option value='A' <?php if ($_GET['dicetak']=='A'){echo "selected"; } ?> >Semua Data</option>
                    <option value='Y' <?php if ($_GET['dicetak']=='Y'){echo "selected"; } ?>>Data yang sudah dicetak</option>
                    <option value='N' <?php if ($_GET['dicetak']=='N'){echo "selected"; } ?>>Data yang belum dicetak</option>
  ...
  if($_GET['aksi']=='proses_tampilkan_data'){
    ...
    }elseif ($_GET['dicetak']=='Y'){
      $sql_t="select * from view_daftar_undangan where dicetak='Y' order by id desc";
    }elseif ($_GET['dicetak']=='N'){
      $sql_t="select * from view_daftar_undangan where dicetak='N' order by id desc";
    }
  }else{
      $sql_t="select * from view_daftar_undangan order by id desc";
  }
  ...
  while ($r=mysql_fetch_array($query)){
    ...
    <tr>
      ...
      echo "<td class='t_center'><input type='checkbox' id='c$s' onclick='javascript: update_data(\"$r[dicetak]\",\"$r[id]\",\"c$s\")' name='cek[]' value='$r[dicetak]' $checked /><label for='c$s'><span></span></label></td>"; ?>
