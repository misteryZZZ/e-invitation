  ...
  var loading = $("#loader");
    ...
    if (a=='N'){
      ...
      $.ajax({
        type:"GET",
        url:"proses.php",
        data:"aksi=proses_update_dikirim&id="+b+"&dikirim="+cek,
...
if ($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  <div class="grid">
                ...
                <div class="grid-title">
                 <div class="pull-left">
                    ...
                    <span style="width: 300px;">Undangan yang dikirim</span> 
                    <div class="clearfix"></div>
                 </div>
                 <div class="pull-right"> 
               ...
               <form id='form_data' method='get' action='<?php echo "media.php?".$_SESSION['query_string']?>'> 
                   ...
                   <input type="hidden" name="module" value="undangan_dikirim" />
                   Tampilkan: <select name='dikirim' id='dikirim'>
                    <option value='A' <?php if ($_GET['dikirim']=='A'){echo "selected"; } ?> >Semua Data</option>
                    <option value='Y' <?php if ($_GET['dikirim']=='Y'){echo "selected"; } ?>>Data yang sudah dikirim</option>
                    <option value='N' <?php if ($_GET['dikirim']=='N'){echo "selected"; } ?>>Data yang belum dikirim</option>
 ...
 if($_GET['aksi']=='proses_tampilkan_data'){
    ...
    }elseif ($_GET['dikirim']=='Y'){
      $sql_t="select * from view_daftar_undangan where dikirim='Y' order by id desc";
    }elseif ($_GET['dikirim']=='N'){
      $sql_t="select * from view_daftar_undangan where dikirim='N' order by id desc";
    }
  }else{
      $sql_t="select * from view_daftar_undangan order by id desc";
  }
  ...
  while ($r=mysql_fetch_array($query)){
    ...
    <tr>
      ...
      echo "<td class='t_center'><input type='checkbox' id='c$s' onclick='javascript: update_data(\"$r[dikirim]\",\"$r[id]\",\"c$s\")' name='cek[]' value='$r[dikirim]' $checked /><label for='c$s'><span></span></label></td>"; ?>
